...
import pojos.Fallecidos;

public class FallecidosCRUD extends javax.swing.JInternalFrame {

    public FallecidosCRUD() {
        initComponents();
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setResizable(true);
        setTitle("Partidas de Defuncion");
        setToolTipText("");
        setVisible(true);
        ...
        jLabel1.setText("Partidas de Defuncion");
        ...
        jLabel6.setIcon(new javax.swing.ImageIcon(getClass().getResource("/imagenes/fallecidos.png"))); // NOI18N
   ...
   // TableRowSorter trs1 ;
    ...
    private void btnProcesarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnProcesarActionPerformed
        ...
        if(RadioButtonAgregar.isSelected())
            ...
            Fallecidos muertos = new Fallecidos();
            EntityTransaction tx = em.getTransaction();
            tx.begin();
            try {
                muertos.setNombre(campoNombre.getText().toUpperCase());
                muertos.setLibro(new BigInteger(campoNumLibro.getText()));
                muertos.setPagina(new BigInteger(campoNumPagina.getText()));
                em.persist(muertos);
                tx.commit();
                } catch (Exception e) {
                    tx.rollback();
        ...
        if(RadioButtonModificar.isSelected())
        {
            ...
            tx.begin();
            try {
               Fallecidos muertos = em.find(Fallecidos.class, idNacido1);
               muertos.setNombre(campoNombre.getText().toUpperCase());
               muertos.setLibro(new BigInteger(campoNumLibro.getText()));
               muertos.setPagina(new BigInteger(campoNumPagina.getText()));
               em.persist(muertos);
    ...
    private void campoBuscoNombreKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_campoBuscoNombreKeyPressed
        ...
        if(evt.getKeyCode()==VK_ENTER) 
        {
           ...
           List<Fallecidos> resultList = em.createNativeQuery("select * from fallecidos  WHERE nombre like '%"+campoBuscoNombre.getText().toUpperCase()+"%'", Fallecidos.class).getResultList();
           for (Fallecidos next : resultList) {
                Object FilaElemento[] = {next.getIdfa(), next.getNombre(), next.getLibro(), next.getPagina() };
    ...
    private static final Logger LOG = getLogger(FallecidosCRUD.class.getName());

    private void cargarDataTabla() {
        EntityManagerFactory emf = createEntityManagerFactory("AlcaldiasPU");
        EntityManager em = emf.createEntityManager();
        DefaultTableModel tablaFactura = (DefaultTableModel) tablaCasados.getModel();
        List<Fallecidos> resultList = em.createQuery("Select m From Fallecidos m ORDER BY m.idfa  asc", Fallecidos.class).getResultList();
        for (Fallecidos next : resultList) 
        {
            Object FilaElemento[] = {next.getIdfa(),next.getNombre(),next.getLibro(),next.getPagina()};

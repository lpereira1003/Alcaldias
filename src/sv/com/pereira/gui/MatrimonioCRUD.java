...
import pojos.Matrimonios;

public class MatrimonioCRUD extends javax.swing.JInternalFrame {

    public MatrimonioCRUD() {
        initComponents();
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setResizable(true);
        setTitle("Partidas de Matrimonios");
        setToolTipText("");
        setVisible(true);
        ...
        jLabel1.setText("Partidas de Matrimonios");
        ...
        jLabel6.setIcon(new javax.swing.ImageIcon(getClass().getResource("/imagenes/matrimonios.png"))); // NOI18N
   ...
   // TableRowSorter trs1 ;
    ...
    private void btnProcesarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnProcesarActionPerformed
        ...
        if(RadioButtonAgregar.isSelected())
            ...
            Matrimonios casados = new Matrimonios();
            EntityTransaction tx = em.getTransaction();
            tx.begin();
            try {
                casados.setNombre(campoNombre.getText().toUpperCase());
                casados.setLibro(new BigInteger(campoNumLibro.getText()));
                casados.setPagina(new BigInteger(campoNumPagina.getText()));
                em.persist(casados);
                tx.commit();
                } catch (Exception e) {
                    tx.rollback();
        ...
        if(RadioButtonModificar.isSelected())
        {
            ...
            tx.begin();
            try {
               Matrimonios casados = em.find(Matrimonios.class, idNacido1);
               casados.setNombre(campoNombre.getText().toUpperCase());
               casados.setLibro(new BigInteger(campoNumLibro.getText()));
               casados.setPagina(new BigInteger(campoNumPagina.getText()));
               em.persist(casados);
    ...
    private void campoBuscoNombreKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_campoBuscoNombreKeyPressed
        ...
        if(evt.getKeyCode()==VK_ENTER) 
        {
           ...
           List<Matrimonios> resultList = em.createNativeQuery("select * from matrimonios  WHERE nombre like '%"+campoBuscoNombre.getText().toUpperCase()+"%'", Matrimonios.class).getResultList();
           for (Matrimonios next : resultList) {
                Object FilaElemento[] = {next.getIdmatri(), next.getNombre(), next.getLibro(), next.getPagina() };
    ...
    private static final Logger LOG = getLogger(MatrimonioCRUD.class.getName());

    private void cargarDataTabla() {
        EntityManagerFactory emf = createEntityManagerFactory("AlcaldiasPU");
        EntityManager em = emf.createEntityManager();
        DefaultTableModel tablaFactura = (DefaultTableModel) tablaCasados.getModel();
        List<Matrimonios> resultList = em.createQuery("Select m From Matrimonios m ORDER BY m.idmatri  asc", Matrimonios.class).getResultList();
        for (Matrimonios next : resultList) 
        {
            Object FilaElemento[] = {next.getIdmatri(),next.getNombre(),next.getLibro(),next.getPagina()};

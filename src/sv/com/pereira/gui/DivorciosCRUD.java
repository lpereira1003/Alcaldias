...
import pojos.Divorcios;

public class DivorciosCRUD extends javax.swing.JInternalFrame {

    public DivorciosCRUD() {
        initComponents();
    ...
    private void initComponents() {
        ...
        tablaCasados = new javax.swing.JTable();
        ...
        setMaximizable(true);
        setResizable(true);
        setTitle("Partidas de Divorcios");
        setToolTipText("");
        setVisible(true);
        ...
        jLabel1.setText("Partidas de Divorcios");
        ...
        tablaCasados.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
        ...
        tablaCasados.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                tablaCasadosMouseClicked(evt);
            }
        });
        jScrollPane2.setViewportView(tablaCasados);
        if (tablaCasados.getColumnModel().getColumnCount() > 0) {
            tablaCasados.getColumnModel().getColumn(0).setMinWidth(60);
            tablaCasados.getColumnModel().getColumn(0).setPreferredWidth(60);
            tablaCasados.getColumnModel().getColumn(0).setMaxWidth(60);
            tablaCasados.getColumnModel().getColumn(2).setMinWidth(100);
            tablaCasados.getColumnModel().getColumn(2).setPreferredWidth(100);
            tablaCasados.getColumnModel().getColumn(2).setMaxWidth(100);
            tablaCasados.getColumnModel().getColumn(3).setMinWidth(100);
            tablaCasados.getColumnModel().getColumn(3).setPreferredWidth(100);
            tablaCasados.getColumnModel().getColumn(3).setMaxWidth(100);
        }
        ...
        jLabel6.setIcon(new javax.swing.ImageIcon(getClass().getResource("/imagenes/Divorcios.png"))); // NOI18N
   ...
   // TableRowSorter trs1 ;
    ...
    private void btnProcesarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnProcesarActionPerformed
        ...
        int fila = tablaCasados.getSelectedRow();
        ...
        if(RadioButtonModificar.isSelected()||(RadioButtonEliminar.isSelected())){
           if(fila==-1){
            ...
            idNacido = tablaCasados.getValueAt(fila, 0).toString();
        ...
        if(RadioButtonAgregar.isSelected())
            ...
            Divorcios divorciados = new Divorcios();
            EntityTransaction tx = em.getTransaction();
            tx.begin();
            try {
                divorciados.setNombre(campoNombre.getText().toUpperCase());
                divorciados.setLibro( Integer.parseInt(campoNumLibro.getText()));
                divorciados.setPagina(Integer.parseInt(campoNumPagina.getText()));
               
                em.persist(divorciados);
                tx.commit();
                } catch (Exception e) {
                    tx.rollback();
                }
        ...
        if(RadioButtonModificar.isSelected())
        {
            ...
            tx.begin();
            try {
               Divorcios divorciados = em.find(Divorcios.class, idNacido1);
               divorciados.setNombre(campoNombre.getText().toUpperCase());
               divorciados.setLibro(Integer.parseInt(campoNumLibro.getText()));
               divorciados.setPagina(Integer.parseInt(campoNumPagina.getText()));
               em.persist(divorciados);
    ...
    private void campoBuscoNombreKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_campoBuscoNombreKeyPressed
        ...
        EntityManagerFactory emf = createEntityManagerFactory("AlcaldiasPU");
        EntityManager em = emf.createEntityManager();
        DefaultTableModel tablaEncontrados = (DefaultTableModel) tablaCasados.getModel();// escribir data encontrada
        if(evt.getKeyCode()==VK_ENTER) 
        {
           DefaultTableModel temp = (DefaultTableModel) tablaCasados.getModel();// elimina data en jtable
           int a = temp.getRowCount();
           for (int i = 0; i < a; i++) {
                temp.removeRow(0);
           }
           List<Divorcios> resultList = em.createNativeQuery("select * from Divorcios  WHERE nombre like '%"+campoBuscoNombre.getText().toUpperCase()+"%'", Divorcios.class).getResultList();
           for (Divorcios next : resultList) {
                Object FilaElemento[] = {next.getIddiv(), next.getNombre(), next.getLibro(), next.getPagina() };
                tablaEncontrados.addRow(FilaElemento);
            }
        }
        int lineasTablaData = tablaCasados.getRowCount();
        tablaCasados.getSelectionModel().setSelectionInterval(lineasTablaData-1,lineasTablaData-1);
    }//GEN-LAST:event_campoBuscoNombreKeyPressed

    private void tablaCasadosMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tablaCasadosMouseClicked
        // TODO add your handling code here:
        int fila = tablaCasados.getSelectedRow();
        if(RadioButtonModificar.isSelected()|| (RadioButtonEliminar.isSelected()))
        {
            campoNombre.setText(tablaCasados.getValueAt(fila, 1).toString());
            campoNumLibro.setText(tablaCasados.getValueAt(fila, 2).toString());
            campoNumPagina.setText(tablaCasados.getValueAt(fila, 3).toString());
        }
    }//GEN-LAST:event_tablaCasadosMouseClicked
    ...
    private javax.swing.JTable tablaCasados;
    ...
    private static final Logger LOG = getLogger(DivorciosCRUD.class.getName());

    private void cargarDataTabla() {
        EntityManagerFactory emf = createEntityManagerFactory("AlcaldiasPU");
        EntityManager em = emf.createEntityManager();
        DefaultTableModel tablaFactura = (DefaultTableModel) tablaCasados.getModel();
        List<Divorcios> resultList = em.createQuery("Select m From Divorcios m ORDER BY m.iddiv  asc", Divorcios.class).getResultList();
        for (Divorcios next : resultList) 
        {
            Object FilaElemento[] = {next.getIddiv(),next.getNombre(),next.getLibro(),next.getPagina()};
        ...
        int lineasTablaData = tablaCasados.getRowCount();
        tablaCasados.getSelectionModel().setSelectionInterval(lineasTablaData-1,lineasTablaData-1);// selecciona ultimo registro
        tablaCasados.scrollRectToVisible(tablaCasados.getCellRect(tablaCasados.getRowCount()-1, 0, true));// va al ultimo registro
    }
